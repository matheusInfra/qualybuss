...
function EmpresaManager() {
  ...
  const [buscandoCnpj, setBuscandoCnpj] = useState(false); // <--- NOVO STATE
  ...
  // --- CONSULTA DE CNPJ (NOVA FUNÇÃO) ---
  const handleCnpjBlur = async (e) => {
    const cnpj = e.target.value.replace(/\D/g, '');
    if (cnpj.length !== 14) return;

    const toastId = toast.loading('Buscando dados na Receita...');
    ...
    try {
      ...
      setValue('nome_fantasia', data.nome_fantasia || data.razao_social); // Fallback se não tiver fantasia
      setValue('data_fundacao', data.data_inicio_atividade);
      ...
      // Contato (Tenta formatar se existir)
      if (data.ddd_telefone_1 && data.telefone_1) {
         setValue('telefone', `(${data.ddd_telefone_1}) ${data.telefone_1}`);
      }
      if (data.email) {
         setValue('email_contato', data.email);
      }
  ...
  const handleSave = async (data) => {
    ...
    try {
      // Garante formatação de data ou null
      const payload = { 
        ...data, 
          ...
          <div className="detail-title">
            ...
            <p className="detail-subtitle">
              {selectedEmpresaId === 'new' 
                ...
                : 'Gerencie os dados cadastrais e fiscais desta unidade.'}
            </p>
          </div>
          {selectedEmpresaId !== 'new' && (
            <button 
          ...
          <div className="detail-card">
            ...
            <div className="erp-grid">
              ...
              <div className="erp-group">
                <label>CNPJ {buscandoCnpj && <span style={{color:'#005A9C', fontSize:'0.8em'}}>(Buscando...)</span>}</label>
                <Controller
                  name="cnpj"
                  control={control}
                  render={({ field }) => (
                    <IMaskInput 
                      ...
                      onBlur={(e) => { field.onBlur(e); handleCnpjBlur(e); }} // <--- GATILHO
                    />
                  )}
                />
              </div>
              ...
              <div className="erp-group">
                ...
                <input className="erp-input" type="date" {...register('data_fundacao')} />
              </div>

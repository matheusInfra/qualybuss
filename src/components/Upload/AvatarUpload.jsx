...
export default function AvatarUpload({ url, onUpload, editable = true }) {
  ...
  const uploadAvatar = async (event) => {
    try {
      ...
      // Captura a URL antiga para deletar depois
      ...
      if (oldAvatarUrl) {
        try {
          ...
          if (parts.length === 2) {
            ...
            if (removeError) console.warn('Não foi possível remover o avatar antigo:', removeError.message);
          }
        } catch (removeErr) {
          console.error('Erro ao tentar remover avatar antigo:', removeErr);
        }
      }
